...
```{r, echo=FALSE}
knitr::include_graphics(file.path(img_path,"kmeans-distance.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(file.path(img_path,"kmeans-centers.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(file.path(img_path,"hclust-one.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(file.path(img_path,"hclust-two.png"))
```

Thus, the algorithm creates a higher hierarchy for this grouping.

```{r, echo=FALSE}
knitr::include_graphics(file.path(img_path,"hclust-three.png"))
```
...
dend_modelo <- as.dendrogram(hierarchical_model)
...
`r ne`. Create the hierarchical model to group using the recommended number of clusters found. Use the `data_posts_norm` object for the creation of the model. Add to the `data_posts_agrupados` object the column `cluster_jer` to store the result of the grouping.
